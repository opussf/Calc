...
function calc.Help()
	...
	calc.Print("   map <statement> - apply <statement> to entire stack.", false)
...
calc.functions = {
	...
	-- ["vers%"] = function() calc.Push( GetCritChance() ) end,
	["vers"] = function() calc.Push( GetCombatRating( CR_VERSATILITY_DAMAGE_DONE ) ) end,
	["str"]  = function() calc.Push( select(2, UnitStat("player", 1))) end,
...
function calc.Map( msg )
	...
	while #reverseStack > 1 do  -- do this one less time than in the stack, since the at the end the msg is appended to the line
		calc.ProcessLine( table.remove( reverseStack ).." "..msg )
	end
	calc.ProcessLine( table.remove( reverseStack ) )
end
